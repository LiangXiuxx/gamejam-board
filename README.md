...
const COLLECTION = "tasks";         // ← Firestore 里放任务的那个集合名
const JAM_START  = "2026-10-01";    // ← 开赛日期（含当天），10/1 开赛
const JAM_DAYS   = 21;              // ← 赛程总天数：10/1 – 10/21
const FREEZE_DAY = 14;              // ← 第几天功能冻结（留 7 天做内容+打磨+提交）
const MEMBERS    = ["策划_三明治","策划_鹿鹿","美术_梦深云","程序_良秀","程序_小草"];
```

改 `JAM_START` / `JAM_DAYS` 一处，顶部徽标、统计卡的剩余天数、卡片截止标签、
甘特图横轴会**全部跟着变**，没有别处写死的天数。
...
- 紫色竖线 = 现在；横轴从 `JAM_START` 起算，长度 = `JAM_DAYS` 天（当前是 10/1 – 10/21）
- 开赛前打开会提示倒计时，并把开赛前的活动挤在横轴最左边（那是测试数据，不是真实排期）

## 人员命名约定

负责人用 **`角色_名字`** 的格式，例如 `策划_三明治`、`程序_良秀`：

- **头像缩写**自动取名字首字（`程序_良秀` → `良`，`美术_梦深云` → `梦`）
- **团队视图的角色**直接取前缀（`策划_三明治` → 显示「策划」），不用额外填字段
- 旧格式（`程序 A`、`PM B`、`美术`）仍然认，只是头像会退化成正则推断的结果

改人名只需要改 `MEMBERS` 数组 + 已有任务的 `owner` 字段。
